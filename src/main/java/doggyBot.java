...
public class doggyBot extends TelegramLongPollingBot {
    ...
    public String playGame1(int input, Update update) {
        ...
        if (computer == 0) { //電腦出包
            ...
            if (input == 3) {//玩家出揼
                result = "你出\uD83D\uDC4A 我出\uD83D\uDD90" + "\n你輸左❌" + "我同你變得更親近";
                dictionary.get(update.getMessage().getFrom().getId()).setRelationship(dictionary.get(update.getMessage().getFrom().getId()).getRelationship() + 2);
            }
        ...
        if (update.hasMessage() && update.getMessage().hasText()) {
            ...
            if (dictionary.get(update.getMessage().getFrom().getId()).isDogAlive()) {
                ...
                if (call_data.equals("food_milk")) {
                    if (!(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 2 > 100)) {
                        if (dictionary.get(update.getMessage().getFrom().getId()).getCurrency() >= 10) {
                            new_message.setText("用$10餵左牛奶\n飢餓值增加左: 2");
                            dictionary.get(update.getMessage().getFrom().getId()).setHunger(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 2);
                            dictionary.get(update.getMessage().getFrom().getId()).setCurrency(dictionary.get(update.getMessage().getFrom().getId()).getCurrency() - 10);
                        } else {
                            new_message.setText(("你無足夠的錢"));
                        }
                    } else {
                        new_message.setText("\uD83D\uDC3E" + dictionary.get(update.getMessage().getFrom().getId()).getName() + "太飽了，遲D再餵佢食野啦");
                    }
                }
                if (call_data.equals("food_dogfood")) {
                    if (!(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 4 > 100)) {
                        if (dictionary.get(update.getMessage().getFrom().getId()).getCurrency() >= 15) {
                            new_message.setText("用$15餵左狗糧\n飢餓值增加左: 4");
                            dictionary.get(update.getMessage().getFrom().getId()).setHunger(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 4);
                            dictionary.get(update.getMessage().getFrom().getId()).setCurrency(dictionary.get(update.getMessage().getFrom().getId()).getCurrency() - 15);
                        } else {
                            new_message.setText(("你無足夠的錢"));
                        }
                    } else {
                ...
                if (call_data.equals("food_meat")) {
                    if (!(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 8 > 100)) {
                        if (dictionary.get(update.getMessage().getFrom().getId()).getCurrency() >= 40) {
                            new_message.setText("用$40餵左肉\n飢餓值增加左: 8");
                            dictionary.get(update.getMessage().getFrom().getId()).setHunger(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 8);
                            dictionary.get(update.getMessage().getFrom().getId()).setCurrency(dictionary.get(update.getMessage().getFrom().getId()).getCurrency() - 40);
                        } else {
                            new_message.setText(("你無足夠的錢"));
                        }
                    } else {
                ...
                if (call_data.equals("food_chocolate")) {
                    if (!(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 30 > 100)) {
                        if (dictionary.get(update.getMessage().getFrom().getId()).getCurrency() >= 150) {
                            new_message.setText("用$150餵左朱古力\n飢餓值增加左: 30");
                            dictionary.get(update.getMessage().getFrom().getId()).setHunger(dictionary.get(update.getMessage().getFrom().getId()).getHunger() + 30);
                            dictionary.get(update.getMessage().getFrom().getId()).setCurrency(dictionary.get(update.getMessage().getFrom().getId()).getCurrency() - 150);
                        } else {
                            new_message.setText(("你無足夠的錢"));
                        }
                    } else {
                        new_message.setText("\uD83D\uDC3E" + dictionary.get(update.getMessage().getFrom().getId()).getName() + "會飽到死嫁");
                    }
                }
                //如果玩家決定殺貓
                if (call_data.equals("kill_yes")) {
                    new_message.setText("\uD83D\uDC3E" + dictionary.get(update.getMessage().getFrom().getId()).getName() + "已經前往西天了\uD83D\uDC80 R.I.P.");
                    dictionary.remove(update.getMessage().getFrom().getId());
                    for (int temp : dictionary.keySet()) {
                        System.out.println(temp);
                    }
